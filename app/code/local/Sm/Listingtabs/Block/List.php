...
class Sm_Listingtabs_Block_List extends Mage_Catalog_Block_Product_Abstract
{
	...
	protected function _toHtml()
	{
		...
		if (!class_exists('Cache_Lite'))	
			require_once($this->getBaseDir() .  'lib' . DS . 'Sm' .DS . 'Listingtabs' .DS .'Cache_Lite' . DS . 'Lite.php');
		$options = array(
		...
		if ($this->_isAjax()) {
			...
			if ($use_cache){
				$cacheid_items = md5(serialize(array ($this->_moduleID(),$this->_getConfig(), $ajax_listingtags_start, $catid)));
				if ( $dataitems = $Cache_Lite->get($cacheid_items)) {
					return  $dataitems;
				} else { 
					$template_file = 'sm/listingtabs/default_items.php';
					$template_file = (!empty($template_file)) ? $template_file : "sm/listingtabs/default_items.php";
			...
			if ($use_cache){
				...
				if ( $data = $Cache_Lite->get($cacheid)) {
					...
					$template_file = (!empty($template_file)) ? $template_file : "sm/listingtabs/default.phtml";
					$this->setTemplate($template_file);
					$data = parent::_toHtml();
	...
	public function getLabel($filter)
	{
		switch ($filter) {
			...
			case 'entity_id':
				return $this->__('Id');
			case 'created_at':
				return $this->__('Created Date');
			case 'price':
				return $this->__('Price');
			case 'lastest_product':
				return $this->__('New Arrivals');
			case 'top_rating':
				return $this->__('Top Rating');
			case 'most_reviewed':
				return $this->__('Most Reviews');
			case 'most_viewed':
				return $this->__('Featured Products');
			case 'best_sales':
				return $this->__('Best Sellers');
		}
	}
