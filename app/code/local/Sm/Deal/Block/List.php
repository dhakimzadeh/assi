...
class Sm_Deal_Block_List extends Mage_Catalog_Block_Product_Abstract
{
	...
	public function _getCfg($attr = null)
	{
		...
		$def_cfgs = Mage::getConfig()
			...
			->getNode('default/deal_cfg')->asArray();
		...
		$_configs = (array)Mage::getStoreConfig("deal_cfg");
		if (empty($_configs)) return;
		$cfgs = array();

		foreach ($groups as $group) {
			$_cfgs = Mage::getStoreConfig('deal_cfg/' . $group . '');
				...
				if ($products != null) {
					...
					if (!empty($_products)) {
						foreach ($_products as $_product) {
							...
							$_product->_image = (string)Mage::helper('catalog/image')->init($_product, 'small_image')->resize($this->_getConfig('imgcfg_width'), $this->_getConfig('imgcfg_height'));
							...
							//tamvt 
							$_product->special_price = $_product->getSpecialPrice();
							$_product->special_from_date = $_product->getSpecialFromDate();
							$_product->special_to_date = $_product->getSpecialToDate();
						}
						return $_products;
					}
				}
