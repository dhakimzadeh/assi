		...
		$.fn.filmore = function(opts, callback) {
			...
			function onLoadVideo() {
				if ($(".filmoreSlide.filmoreCurrent", target)) {
					var src_iframe = $("iframe", opts.slide_id).attr('src');
					...
					$("iframe", opts.slide_id).removeAttr('src');
					$("iframe", opts.slide_id).attr('src', src_iframe);
					...
					$(".video_html5_cameraslide", opts.slide_id).append('<video width=' + width_html5 + ' height=' + height_html5 + ' ' + controls + ' ' + autoplay + ' ' + muted + ' ' + loop + ' style="' + style_html5 + '">' + source + '</video>');
				}
			}
